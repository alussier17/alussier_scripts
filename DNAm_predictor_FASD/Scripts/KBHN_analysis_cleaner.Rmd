...
*161 probes*
```{r}
validated.hits <- read.csv("~/Chudley_NDN/validated.hits.annotated.csv", row.names = 1)

set.seed(17)
gbmFit.161.all <- train(y=factor(ndn1.groups, levels = c("FASD","Control")),
                 x = (training.ndn1[,which(colnames(training.ndn1) %in% rownames(validated.hits))]), 
                 method = "gbm", trControl = fitControl.2, 
                 verbose = FALSE, tuneGrid = gbmGrid, metric = "ROC")
gbmFit.161.all$finalModel
set.seed(17)
gbmFit.161.FASD <- train(y=factor(ndn1.groups.2, levels = c("FASD","Control")),
                 x = (training.ndn1.2[,which(colnames(training.ndn1.2) %in% rownames(validated.hits))]), 
                 method = "gbm", trControl = fitControl.2, 
                 verbose = FALSE, tuneGrid = gbmGrid, metric = "ROC")
gbmFit.161.FASD$finalModel

plot(gbmFit4)
plot(varImp(gbmFit4))

whichTwoPct.4 <- tolerance(gbmFit4$results, metric = "ROC", tol = 2, maximize = TRUE)  
gbmFit4$results[whichTwoPct.4,1:10]
gbmFit4$bestTune

gbmFit4.top.pred <- gbmFit4$pred[(which(gbmFit4$pred$n.trees == gbmFit4$bestTune$n.trees & 
                                          gbmFit4$pred$interaction.depth == gbmFit4$bestTune$interaction.depth)),]
gbmFit4.top.pred.roc <- roc(predictor = gbmFit4.top.pred$FASD, response = gbmFit4.top.pred$obs)
gbmFit4.top.pred.roc

gbmFit4.top.pred$Fold <- strsplit2(gbmFit4.top.pred$Resample, "\\.")[,1]
gbmFit4.top.pred$Rep <- strsplit2(gbmFit4.top.pred$Resample, "\\.")[,2]


head(gbmFit4.top.pred)
ggplot(gbmFit4.top.pred,aes(d = obs, m= 1-FASD, colour = Fold))+
  geom_roc(n.cuts = 0, labels = F)+
  scale_color_manual(values = rep("grey",10))+
  geom_roc(data = gbmFit4.top.pred, aes(d = obs, m= 1-FASD), color = "black", labels = F, n.cuts =0)+
  ggtitle("ROC of training data")+
  #annotate("text", x = .75, y = .25, 
  #        label = paste("AUC =", round(gbmFit4.top.pred.roc$auc, 3))) +
  style_roc(xlab = "1 - Specificity")+
  theme(axis.title.y = element_text(size=25, face="bold", vjust=1.2),
        axis.title.x = element_text(size=25, face="bold", vjust=1.2),
        axis.text.y = element_text(size=18, color="black"),
...
ci(gbmFit4.top.pred.roc)

set.seed(17)
confusionMatrix(predict(gbmFit4, newdata = training.ndn1.2[,which(colnames(training.ndn1.2) %in%
                                                                     rownames(validated.hits))]), 
                factor(ndn1.groups.2, levels = c("FASD","Control")))

confusionMatrix(predict(gbmFit4, newdata = (test.ndn2[,which(colnames(test.ndn2) %in% rownames(validated.hits))])),
                factor(pData(chudley_clean)$Group, levels = c("FASD","Control")))


gbm4.votes <- predict(gbmFit4, newdata = (test.ndn2[,which(colnames(test.ndn2) %in% rownames(validated.hits))]), 
                      type = "prob")
gbm4.pred <- gbm4.votes[,2]

histogram(~gbm4.votes$Control|factor(pData(chudley_clean)$Group, levels = c("FASD","Control")), 
          xlab="Probability of Poor Segmentation")
gbm4.ROC <- roc(predictor=gbm4.votes$FASD,
            response=as.factor(pData(chudley_clean)$Group))
str(gbm4.ROC)

plot(gbm4.ROC, main = "ROC curve of NDN II\nArea under the curve: 0.9427")
auc(gbm4.ROC)
str(gbm4.ROC)
resampleHist(gbmFit4)

gbm4.ROC.plot <- data.frame(predictor=gbm4.votes$FASD,
            response=as.factor(pData(chudley_clean)$Group))
ggplot(gbm4.ROC.plot, aes(d = response, m = predictor))+
  geom_roc(n.cuts = 0, labels = F)+
  geom_rocci(labels =F, size=0, sig.level = 0.05)+
  ggtitle("ROC of test data (NDN II)")+
  #annotate("text", x = .75, y = .25, 
  #         label = paste("AUC =", round(gbm4.ROC$auc, 3))) +
  style_roc(xlab = "1 - Specificity")+
    theme(axis.title.y = element_text(size=25, face="bold", vjust=1.2),
        axis.title.x = element_text(size=25, face="bold", vjust=1.2),
        axis.text.y = element_text(size=18, color="black"),
...
roc.test(gbmFit4.top.pred.roc, gbm4.ROC)
str(gbmFit4.top.pred.roc)
```
...
head(gbmFit4.top.pred)
head(gbm4.ROC.plot.2)

gbm4.ROC.plot.2 <- gbm4.ROC.plot
gbm4.ROC.plot.2$Fold = "validation"

gbmFit4.top.pred.2 <- gbmFit4.top.pred[,c(2,4,11)]
gbmFit4.top.pred.2$FASD <- (1-gbmFit4.top.pred.2$FASD)
colnames(gbmFit4.top.pred.2) <- c("response","predictor","Fold")

gbm4.ROC.plot.3 <- rbind(gbm4.ROC.plot.2, gbmFit4.top.pred.2[,c(2,1,3)])

ggplot(gbm4.ROC.plot.3, aes(d = response, m = 1-predictor, color = Fold))+
  geom_roc(data =gbm4.ROC.plot.3[grep("Fold", gbm.ROC.plot.3$Fold),], n.cuts = 0, labels = F, lwd = .7)+
  scale_color_manual(values = rep("lightgrey",10))+
  geom_roc(data = gbm4.ROC.plot.3[grep("Fold", gbm.ROC.plot.3$Fold),], aes(d = response, m= 1-predictor), 
           color = "#545454", labels = F, n.cuts =0, lwd =1.3)+
  geom_roc(data = gbm4.ROC.plot.3[grep("validation", gbm.ROC.plot.3$Fold),], aes(d = response, m= predictor), 
           color = "blue", labels = F, n.cuts =0, lwd = 1.3)+
  style_roc(xlab = "1 - Specificity")+
  theme(axis.title.y = element_text(size=25, face="bold", vjust=1.2),
        axis.title.x = element_text(size=25, face="bold", vjust=1.2),
...
*Testing on other Kobor datasets*
```{r}
#Testing on Drakenstein - blood#
load("~/2016.01.20drak_final_noreps.Rdata")
dim(drak.final)
drak.final.betas.pred <- betas(drak.final[which(featureNames(drak.final) %in% rownames(validated.hits)),])
dim(drak.final.betas.pred) #128 samples, 159 probes
drak.predict <- predict(gbmFit4, t(drak.final.betas.pred))
length(drak.predict == "FASD") #128 of 128 -.- 


###TESTING ON GECKO (8-10 years old, M/F, typically developing) FROM SARAH GOODMAN ###
load("~/GECKO_buccal_betas_Alex.RData")
load("~/GECKO_buccal_betas_unnorm_Alex.RData")
dim(GECKO_buccal_betas_unnorm)
gecko.predict.betas <- GECKO_buccal_betas[which(rownames(GECKO_buccal_betas) %in% elodie.hits$Probe),] #146 of 161
dim(gecko.predict.betas) #146 of 161
set.seed(17)
gecko.predicted <- predict(gbmFit3, t(gecko.predict.betas))
length(which(gecko.predicted =="FASD")) ##DOESN'T WORK!! 
#is it because it's only 146 probes? 
test.146.cpg <- do.call(rbind, lapply(1:1000, function(x) {
  print(x)
  set.seed(x)
  cpg.list <- sample(rownames(chel.sv.sig.cor), 146)
  pred.cpg <- gset[which(featureNames(gset) %in% cpg.list),]
  pred.cpg.betas <- m2beta(exprs(pred.cpg))
  asd.predict <- predict(gbmFit3, t(pred.cpg.betas))
  data.frame(seed = x, FASD_num = length(which(asd.predict =="FASD")))
}))

plot(density(test.146.cpg$FASD_num))
#yuppppp...... guess we need all 161 probes (or the 151 non-zero influence)


#C3ARE
load("~/CARE_betas.RData")
dim(CARE_betas) #96 samples
care.predict.betas <- CARE_betas[which(rownames(CARE_betas) %in% elodie.hits$Probe),]
dim(care.predict.betas) #157 of 161
set.seed(17)
care.predicted <- predict(gbmFit3, t(care.predict.betas))
length(which(care.predicted =="FASD")) #2 of 96 
confusionMatrix(care.predicted, factor(rep("Control",96), levels = c("FASD","Control")))

#MAVAN III
load("~/MAVANIII_betas.RData")
dim(MAVANIII_betas) #240 samples
m3.predict.betas <- MAVANIII_betas[which(rownames(MAVANIII_betas) %in% elodie.hits$Probe),]
dim(m3.predict.betas) #161 of 161
set.seed(17)
m3.predicted <- predict(gbmFit3, t(m3.predict.betas))
length(which(m3.predicted =="FASD")) #1 of 240
confusionMatrix(m3.predicted, factor(rep("Control",240), levels = c("FASD","Control")))

#WISCONSIN 18
load("~/WSFW_450k_betas_processed.RData")
dim(WSFW_450k_betas_processed) #189 samples
wisc.predict.betas <- WSFW_450k_betas_processed[which(rownames(WSFW_450k_betas_processed) %in% elodie.hits$Probe),]
dim(wisc.predict.betas) #161 of 161
set.seed(17)
wisc.predicted <- predict(gbmFit3, t(wisc.predict.betas))
length(which(wisc.predicted =="FASD")) #37/189 (19.6% false positive)
confusionMatrix(wisc.predicted, factor(rep("Control",189), levels = c("Control","FASD")))

#ALL BUCCAL
load("~/buccal_betas_preprocessed_BMIQ_2016Sept7.RData")
dim(buccal_betas_preprocessed_BMIQ_2016Sept7) #1191 samples
all.buc <- buccal_betas_preprocessed_BMIQ_2016Sept7[,-(which(colnames(buccal_betas_preprocessed_BMIQ_2016Sept7) 
                                                             %in% c(colnames(final2), colnames(chudley_clean))))]
head(all.buc) #944 samples
all.buc.predict.betas <- all.buc[which(rownames(all.buc) %in% elodie.hits$Probe),]
head(all.buc.predict.betas) #157 of 161
set.seed(17)
all.buc.predicted <- predict(gbmFit3, t(all.buc.predict.betas))
length(which(all.buc.predicted =="FASD")) #5/1191
confusionMatrix(all.buc.predicted, factor(rep("Control",944), levels = c("Control","FASD")))

#PAWS
load("~/PAWS_betas_metas_normalized_combatted_SES_genoPCs_cell_comp.RData")
PAWS_Beta.pred <- PAWS_Beta[which(rownames(PAWS_Beta) %in% colnames(training.ndn1)),]
dim(PAWS_Beta.pred) #161/161 - 177 samples
paws.predicted <- predict(gbmFit3, t(PAWS_Beta.pred))
length(which(paws.predicted =="FASD")) #20/177
confusionMatrix(paws.predicted, factor(rep("Control",177), levels = c("FASD","Control")))

summary(factor(PAWS_meta$ETHNICITY[which(paws.predicted =="FASD")]))
summary((PAWS_meta[which(paws.predicted =="FASD"),]))
summary(factor(PAWS_meta$ETHNICITY))

plot(density(na.omit(PAWS_meta$Sarah_comp_SES)))
lines(density(PAWS_meta$Sarah_comp_SES[which(paws.predicted =="FASD")]))

plot(density(PAWS_meta$PredictedCD34),main = "Density of predicted CD34 proportions", xlab = "CD34 proportion")
lines(density(PAWS_meta$PredictedCD34[which(paws.predicted =="FASD")]), col = "red")
plot(density(PAWS_meta$PredictedBuccal),main = "Density of predicted buccal proportions", xlab = "Buccal proportion")
lines(density(PAWS_meta$PredictedBuccal[which(paws.predicted =="FASD")]), col = "red")

paws.predicted.2 <- predict(gbmFit3, t(PAWS_Beta.pred), type = "prob")
densityplot(~paws.predicted.2$Control|factor(rep("Control",177), levels = c("FASD","Control")), 
          xlab="Probability of Poor Segmentation")
